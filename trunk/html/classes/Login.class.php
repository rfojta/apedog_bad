		...
		if (isset($post['userid'])) {
			...
			if( $count == 1 ) {
				...
				if( $pass == $post['userpass'] ) {
					...
					$row = mysql_fetch_row($res);
			$insert="insert into login_log (`login_inserted`,`pass_inserted`,`successful`) values('".$post['userid']."','".$post['userpass']."','1');";

					mysql_query($insert);
					return array(1,$row[0]);
				}
			}
			$insert="insert into login_log (`login_inserted`,`pass_inserted`,`successful`) values('".$post['userid']."','".$post['userpass']."','0');";
			mysql_query($insert);
